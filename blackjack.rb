class Blackjack
  ...
  def playgame
    ...
    ask_to_hit
    dealer_play unless player.bust? #if the player bust then the dealer is not required to play
  end
  ...
  def determine_winner
    if !dealer.bust? # checks if the dealer did not bust
      ...
      if player.point_total < dealer.point_total then puts "Sorry #{player.name}, you lose. The dealer beat you by #{dealer.point_total - player.point_}" end
